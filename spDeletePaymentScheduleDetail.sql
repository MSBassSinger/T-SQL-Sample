/******************************************************************************************************************************
STORED PROCEDURE: spDeletePaymentScheduleDetail
VERSION: 1.0.0

This SP marks a payment schedule detail record as deleted (tblPaymentScheduleDetails).  
...
IF EXISTS (SELECT name FROM dbo.sysobjects WHERE name = 'spDeletePaymentScheduleDetail')
	DROP PROCEDURE spDeletePaymentScheduleDetail
GO 

CREATE PROCEDURE spDeletePaymentScheduleDetail
(
	@ID               BIGINT,
	@MarkDeleted      BIT,
	...
	*/
	...
	BEGIN TRY	
		...
		IF @NumRows = 0 
			BEGIN;
				SET @ErrNum = 50004;
				SET @throwMsg = FORMATMESSAGE('The attempt to set the IsDeleted field for the payment schedule detail record (ID=%i) failed. Either the record does not exist or you do not have access to that record.', @ID);
...
EXEC sys.sp_addextendedproperty 
	...
	@level1name=N'spDeletePaymentScheduleDetail'
