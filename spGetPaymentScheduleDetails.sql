/******************************************************************************************************************************
STORED PROCEDURE: spGetPaymentScheduleDetails
VERSION: 1.0.0

This SP selects detail data for a payment schedule (from tblPaymentScheduleDetails).  There is a flag to include records marked as deleted.
...
IF EXISTS (SELECT name FROM dbo.sysobjects WHERE name = 'spGetPaymentScheduleDetails')
	DROP PROCEDURE spGetPaymentScheduleDetails
GO 

CREATE PROCEDURE spGetPaymentScheduleDetails
(
	@ParentID               BIGINT,
	@IncludeDeleted         BIT,
	...
	*/
	...
	BEGIN TRY	
		...
		SELECT 
			...
			[IsDeleted],
			[WhenUpdated]
		FROM [dbo].[tblPaymentScheduleDetails] (NOLOCK)
		...
		IF @NumRows = 0 
			BEGIN;
				SET @ErrNum = 50004;
				SET @throwMsg = FORMATMESSAGE('The attempt to access (ParentID=%s) failed. Either the record does not exist or you do not have access to that record.', @ParentID);
...
EXEC sys.sp_addextendedproperty 
	...
	@level1name=N'spGetPaymentScheduleDetails'
